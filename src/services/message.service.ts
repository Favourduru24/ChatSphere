...
export const sendMessageService = async (userId: string,
  ...
  req: Request, ) => {
    ...
    if(replyToId) {
        ...
        return {userMessage: newMessage, aiResponse, isAiChat: chat.isAiChat, chat} 
    }

    async function getAIResponse (chatId: string, userId: string) {
      const chatSphereAI = await UserModel.findOne({isAI: true})
      if(!chatSphereAI) throw new Error('AI user not found')

        const chatHistory = await getChatHistory(chatId)

      const formattedMessage: ModelMessage[] = chatHistory.map((msg: any) => {
         ...
         const result = await streamText({
           ...
           system: 'You are ChatSphere Ai, a helpful and friendly assistant. Respond only with text and attend to the last user message only',
     })
